...
#else 
  constexpr int vlen=32; // MAtch current behaviour
#endif
void test_simd(int N_in, int M, int R, double a) {

#if defined KOKKOS_ENABLE_CUDA
  using simd_t = simd::simd<double,simd::simd_abi::cuda_warp<vlen>>;
#elif defined KOKKOS_ENABLE_HIP
  using simd_t = simd::simd<double,simd::simd_abi::hip_wavefront<vlen>>;
#else
...
int main(int argc, char* argv[]) {
  ...
  if(N%vlen) {
    printf("Please choose an N dividable by %d\n",vlen);
    return 0;
  }
